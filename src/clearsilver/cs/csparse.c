...
static NEOERR *include_parse (CSPARSE *parse, int cmd, char *arg)
{
  ...
  {
    if (arg1.argexpr) free(arg1.argexpr);
    if (arg1.expr1) dealloc_arg (&(arg1.expr1));
    if (arg1.expr2) dealloc_arg (&(arg1.expr2));
    if (arg1.next) dealloc_arg (&(arg1.next));
    return STATUS_OK;
  }
  do {
    err = increase_stack_depth (parse);
...
static NEOERR *def_parse (CSPARSE *parse, int cmd, char *arg)
{
  ...
  */
  ...
  arg++;
  s = arg;
  while (x < ((int)sizeof(name) - 1) && *s && *s != ' ' && *s != '#' && *s != '(')
  {
    name[x++] = *s;
    s++;
  }
